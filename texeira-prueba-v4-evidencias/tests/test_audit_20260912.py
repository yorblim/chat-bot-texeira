...
async def main():
    ...
    await check('¿Qué incluye Machu Picchu en tren?',lambda r:
        'Bus de subida y bajada' in r['response']
        and r['response'].count('Tren de ida y vuelta')==1
        and r['response'].count('Entrada a Machu Picchu')==1
        and 'Tickets de tren' not in r['response']
        and 'Recojo del hotel' in r['response']
        and {'F1','F3'} <= set(r['sources_used']),'follow')
