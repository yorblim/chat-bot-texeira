...
def detect_conflicts(entity_id: str, field: str = None) -> List[dict]:
    ...
    return existing_conflicts + auto_conflicts
...
def build_context_for_entity(entity_id: str) -> str:
    ...
    for (field, item), fact_list in consolidated.items():
        ...
        if item:
            ...
            if isinstance(val, bool):
                ...
                lines.append(f"- {field}: {val} (fuente: {source_str})")
    ...
    if conflicts:
        ...
        for c in conflicts:
            ...
            lines.append(f"- {c['field']}: {src_a['source_id']}={val_a} vs {src_b['source_id']}={val_b}")
