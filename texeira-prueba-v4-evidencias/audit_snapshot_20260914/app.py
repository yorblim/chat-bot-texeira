...
if _whatsapp_enabled and not all(os.environ.get(k) for k in ('META_VERIFY_TOKEN','META_ACCESS_TOKEN','META_PHONE_NUMBER_ID','META_APP_SECRET')):
    raise RuntimeError('Falta configuración de Meta. Revisar el archivo local de credenciales sin compartirlo.')
os.environ['SQLITE_DB_PATH'] = str(_trial_root / 'trial_logs.db')
os.environ['CHROMA_HYBRID_DIR'] = str(_trial_root / 'chroma_v4_evidencias_db')
...
async def receive_message(request: Request, background_tasks: BackgroundTasks):
    """Recepcion de mensajes de Meta (WhatsApp/Messenger)."""
    ...
    try:
        ...
        def _process_message():
            try:
                ...
                elapsed = (time.time() - start_time) * 1000
                print(f"[WA PROCESSED] user_id={user_id} latency={elapsed:.0f}ms route={rag_result.get('response_route', 'unknown')}")
            except Exception as e:
                elapsed = (time.time() - start_time) * 1000
                print(f"[WA ERROR] user_id={user_id} error={e} latency={elapsed:.0f}ms")
...
async def test_chat(request: TestChatRequest):
    ...
    try:
        ...
        escalated_to_human = False  # Esta copia no ejecuta transferencias a asesores.
