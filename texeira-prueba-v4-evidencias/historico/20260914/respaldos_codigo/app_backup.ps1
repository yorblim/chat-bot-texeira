...
_provider_values = dotenv_values(_trial_root.parent / 'texeira-chatbot' / '.env')
for _key in ('LLM_PROVIDER', 'LLM_MODEL', 'GROQ_API_KEY', 'GROQ_API_BASE', 'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'DEEPSEEK_API_KEY'):
    if _provider_values.get(_key):
        os.environ[_key] = _provider_values[_key]
del _provider_values
for _key in ('META_VERIFY_TOKEN', 'META_ACCESS_TOKEN', 'META_PHONE_NUMBER_ID', 'META_APP_SECRET', 'WHATSAPP_TEST_BSUID_MAP'):
    os.environ[_key] = ''
os.environ['WHATSAPP_TEST_MODE'] = 'false'
os.environ['SQLITE_DB_PATH'] = str(_trial_root / 'trial_logs.db')
os.environ['CHROMA_HYBRID_DIR'] = str(_trial_root / 'chroma_provisional_db')
...
CATALOG_PATH = os.path.join(os.path.dirname(__file__), "data", "provisional.json")
...
def needs_escalation(response: str) -> bool:
    ...
    Evita falsos positivos en negaciones como "no prometer que un asesor te contactará".
    """
    import re as _re
    ...
    for indicator in escalation_indicators:
        ...
        if idx >= 0:
            # Verificar si está en una negación: buscar patrones negativos en la misma oración
            ...
            # Usar word boundaries para evitar "humano" matchando "no"
            negation_patterns = [
...
# Guardar en historial
        ...
        if any(kw in q_lower for kw in ["yape", "adelanto", "pago", "pagar", "forma de pago", "depósito", "deposito"]):
            needs_agency_conf = True
        if any(kw in q_lower for kw in ["cupo", "cupos", "disponibilidad", "reserva", "reservar", "confirmar reserva"]):
            needs_agency_conf = True
        if any(kw in q_lower for kw in ["exacto en soles", "en soles", "precio en pen", "tipo de cambio", "conversión", "conversion"]):
            needs_agency_conf = True
        ...
        if "fuentes de precios" in response_text.lower() or "enlaces de las fuentes" in response_text.lower():
            # Reemplazar mención a fuentes de precios con contacto de la agencia
            import json
            from pathlib import Path
            ...
            if catalog_path.exists():
                ...
                response_text = response_text.replace("¿Te gustaría que te proporcione los enlaces de las fuentes de precios para que puedas contactar a la agencia?", "")
                response_text = response_text.replace("te recomiendo contactar directamente a la agencia publicada en las fuentes de referencia.", f"te recomiendo contactar directamente a la agencia:{agency_contact}")
                response_text = response_text.replace("Consultar las políticas de reserva, cancelación y pagos directamente con ellos, ya que esta información no está confirmada en el contexto actual.", f"Consultar las políticas de reserva, cancelación y pagos directamente con la agencia:{agency_contact}")
        ...
        if "prohibición de conversiones" in response_text.lower():
            # Reemplazar多种 variantes del patrón
            response_text = response_text.replace(
                "Prohibición de conversiones",
                "Tipo de cambio no confirmado"
            )
            # Reemplazar la explicación de la prohibición (case-insensitive)
