...
INDEX = ROOT / 'chroma_provisional_db'
NAMES = ['City Tour Cusco', 'Valle Sagrado completo', 'Machu Picchu en tren', 'Laguna Humantay', 'Montaña de 7 Colores', 'Salkantay 4D/3N', 'Paquete Cusco 7D/6N (hipotético)']
NOTICES = {
...
def fact_sheet(t, name):
 lo, hi = t['range_usd']
 return '\n'.join([
  name, f'Rango estimado: USD {lo}–{hi} por adulto, servicio compartido. NO tarifa oficial.',
  ...
  'ESCENARIO SIMULADO, incluye: ' + '; '.join(t['simulation_includes']),
  'ESCENARIO SIMULADO, excluye: ' + '; '.join(t['simulation_excludes']),
  ...
  'Fuentes de precios: ' + ' '.join(CATALOG['sources'][s] for s in t['market_sources']),
 ])
...
def documents():
 ...
 for t, name in zip(CATALOG['tours'], NAMES):
  for i, chunk in enumerate(splitter.split_text(fact_sheet(t, name))):
   rows.append({'page_content': NOTICES['es'] + '\n' + chunk, 'metadata': {'tour_id': t['id'], 'tour_name': name, 'chunk_index': i, 'source': 'provisional-2026-09-11', 'category': 'simulation', 'price_kind': 'reference_range', 'range_min_usd': t['range_usd'][0], 'range_max_usd': t['range_usd'][1]}})
 agency = CATALOG['agency']
 rows.append({'page_content': NOTICES['es'] + '\nContacto publicado: ' + agency['published_phone'] + ' / ' + agency['published_secondary_phone'] + '\nDirección publicada: ' + agency['published_address'] + '\nCorreo, pagos, adelantos, descuentos, cancelación y disponibilidad: sin confirmar. No se pueden confirmar reservas. No existe asignación automática de asesor.', 'metadata': {'tour_id': 'agency', 'tour_name': 'Agencia: datos publicados y pendientes', 'chunk_index': 0, 'source': 'provisional-2026-09-11'}})
 return rows
...
Pregunta: {question}'''
 ...
 def listing(message, lang='es'):
  ...
  if normalize(message).strip(' ?¿!.') not in general:
   return None
  return NOTICES.get(lang, NOTICES['es']) + '\n\n' + '\n'.join(f"- {name}: USD {t['range_usd'][0]}–{t['range_usd'][1]}" for t,name in zip(CATALOG['tours'],NAMES))
 ...
 def chain(question, user_id='default'):
  ...
  aliases = [['city tour','city-tour'],['valle sagrado','sacred valley'],['machu picchu','machupicchu'],['humantay'],['colores','rainbow','vinicunca'],['salkantay'],['7 dias','7 days','7d/6n','paquete completo']]
  ...
  # Responder contacto sin LLM; las consultas complejas se resuelven con el índice provisional.
