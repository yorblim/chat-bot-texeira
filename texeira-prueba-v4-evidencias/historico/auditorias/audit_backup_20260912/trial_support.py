...
def install(ns):
    ...
    ns['SYSTEM_PROMPT'] = '''Eres el asistente de un PROTOTIPO de Texeira Travel. Responde en el idioma del usuario.
...
Si un campo es "unknown" o "por confirmar", NO inventes datos. Indica que falta información.
La ausencia de un dato NO significa que esté excluido. Solo lo que se confirma explícitamente como excluido lo está.
Los productos "confirmed_product=true" significan que Texeira los documenta, NO que todos sus datos estén confirmados.
No inventes políticas, seguros, descuentos, disponibilidad, importes PEN ni conversiones.
No confirmes reservas ni prometas que un asesor contactará al usuario.
...
Pregunta: {question}'''
    ...
    def chain(question, user_id='default'):
        ...
        if any(x in q for x in unconfirmed_keywords):
            text = ('PROTOTIPO: el Paquete Cusco 7D/6N NO está documentado en ninguna fuente de la agencia. '
        ...
        if EVIDENCE_AVAILABLE:
            ...
            if entity_id and not is_product_confirmed(entity_id):
                text = (f'PROTOTIPO: el producto "{entity_id}" no está confirmado como tour de Texeira Travel. '
                        'Consulta con la agencia: ' + CATALOG['agency']['phones'][0] + '.')
                ns['add_to_history'](user_id, 'human', question)
                ns['add_to_history'](user_id, 'ai', text)
                return {'response': text, 'context_used': True, 'is_predefined': False, 'is_fallback': False,
...
def retriever():
    ...
    if marker['catalog_sha256'] != hashlib.sha256((ROOT / 'data/tours_catalog.json').read_bytes()).hexdigest():
        raise RuntimeError('El catalogo cambio: crear otro indice antes de continuar.')
