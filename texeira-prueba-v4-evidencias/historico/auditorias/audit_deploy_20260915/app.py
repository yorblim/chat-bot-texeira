...
_whatsapp_enabled = os.getenv('TEXEIRA_ENABLE_WHATSAPP', 'false').lower() == 'true'
_messenger_enabled = os.getenv('TEXEIRA_ENABLE_MESSENGER', 'false').lower() == 'true'
for _key in ('META_VERIFY_TOKEN', 'META_ACCESS_TOKEN', 'META_PHONE_NUMBER_ID', 'META_APP_SECRET', 'WHATSAPP_TEST_BSUID_MAP', 'WHATSAPP_TEST_MODE'):
    os.environ[_key] = (_provider_values.get(_key) or '') if _whatsapp_enabled else ''
for _key in ('FB_PAGE_ACCESS_TOKEN', 'FB_APP_SECRET'):
    os.environ[_key] = (_provider_values.get(_key) or '') if _messenger_enabled else ''
if _messenger_enabled and not _whatsapp_enabled:
    os.environ['META_VERIFY_TOKEN'] = _provider_values.get('META_VERIFY_TOKEN') or ''
del _provider_values
if _whatsapp_enabled and not all(os.environ.get(k) for k in ('META_VERIFY_TOKEN','META_ACCESS_TOKEN','META_PHONE_NUMBER_ID','META_APP_SECRET')):
    raise RuntimeError('Falta configuración de Meta/WhatsApp. Revisar el archivo local de credenciales sin compartirlo.')
os.environ['SQLITE_DB_PATH'] = str(_trial_root / 'trial_logs.db')
os.environ['CHROMA_HYBRID_DIR'] = str(_trial_root / 'chroma_f1_confirmado_20260915_db')
